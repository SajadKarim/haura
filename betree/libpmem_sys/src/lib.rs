...
mod tests {
    ...
    const DEST_FILEPATH: &str = "/mnt/pmemfs0/unittest\0";
    ...
    fn sample_test() {
        unsafe {
            ...
            dest_filehandle = match libpmem::pmem_file_create(&DEST_FILEPATH, 16*1024*1024, &mut mapped_len, &mut is_pmem) {
            ...
            libpmem::pmem_file_read(&dest_filehandle, 0, &mut buffer, TEXT.chars().count());
            println!("\n TEXT length: {}", TEXT.chars().count());

            let mut buffer2 = vec![0; TEXT2.chars().count()];
            libpmem::pmem_file_read(&dest_filehandle, TEXT.chars().count(), &mut buffer2, TEXT2.chars().count());
            ...
            libpmem::pmem_file_read(&dest_filehandle, 1000, &mut buffer3, TEXT.chars().count());
            println!("\n TEXT length: {}", TEXT.chars().count());
